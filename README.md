...
十二款雙人棋類遊戲的純靜態網頁版。首頁、遊戲畫面、設定、勝率條、行動日誌、規則分頁與返回上一步採用統一介面，可直接部署到 GitHub Pages。
...
| 動物將棋 Let’s Catch the Lion! | `games/animal-shogi/game.html` | `games/animal-shogi/rules.html` |
...
GitHub Actions 會在每次推送到 `main` 或建立 Pull Request 時，自動執行規則、MCTS、十二款共用頁面與內部圖片／連結檢查。
