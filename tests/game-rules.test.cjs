...
(async () => {
  ...
  assert.deepEqual([...openingTray.matchAll(/data-slot="(\d+)"/g)].map((match) => Number(match[1])), [...Array.from({ length: 14 }, (_, slot) => slot), ...Array.from({ length: 14 }, (_, offset) => 27 - offset)], 'The second row reverses direction to form a visual loop');
  ...
  assert.match(shellCss, /\.queue-track \{[^}]*grid-template-columns: repeat\(14, minmax\(0, 1fr\)\);/, 'Chocolate Clash uses a fixed 14 by 2 ring layout');
