...
(async () => {
  ...
  assert.doesNotMatch(classicHint.tray, /classic-level-guide|4級最大・3級・2級・1級最小/, 'Classic Gobblet reserve area omits the level guide row');
  assert.doesNotMatch(classicHint.tray, /<small>(?:[1-4]級|已用完)<\/small>/, 'Classic Gobblet reserve buttons omit visible level text');
  ...
  assert.deepEqual(chocolateGame.openings.map((opening) => opening.value), ['standard', 'random', 'same']);
  assert.equal(chocolateGame.defaultOpening, 'random', 'Chocolate Clash defaults to a newly shuffled ring');
  assert.equal(chocolateGame.openingLabel, '初始設置');
  ...
  assert.deepEqual(chocolate.queue.map((piece) => piece.slot), Array.from({ length: 28 }, (_, slot) => slot), 'Chocolate Clash keeps one fixed visual slot per chocolate');
  ...
  const chocolateCodes = { round: 'A', square: 'B', heart: 'C', flower: 'D' };
  const chocolateClassic = chocolateGame.create('standard', null);
  assert.equal(chocolateClassic.state.queue.map((piece) => chocolateCodes[piece.type]).join(''), 'ABBCCCDAABBBCDDAAABCCDDDABCD', 'Chocolate Clash classic mode uses the requested ring');
  assert.deepEqual(chocolateGame.create('same', chocolateCreated.snapshot).state.queue, chocolate.queue, 'Same mode reuses the previous initial ring');
  const savedRandom = Math.random;
  Math.random = () => .999999;
  const reshuffledChocolate = chocolateGame.create('random', chocolateClassic.snapshot).state.queue;
  Math.random = () => 0;
  const rerolledChocolate = chocolateGame.create('random', chocolateClassic.snapshot).state.queue;
  Math.random = savedRandom;
  assert.notDeepEqual(reshuffledChocolate.map((piece) => piece.type), chocolateClassic.state.queue.map((piece) => piece.type), 'Random mode ignores the previous snapshot and shuffles again');
  assert.notDeepEqual(rerolledChocolate.map((piece) => piece.type), reshuffledChocolate.map((piece) => piece.type), 'Every random new game builds a fresh shuffled ring');
  ...
  const openingTray = chocolateGame.view(chocolate, {}).tray;
  const openedTray = chocolateGame.view(chocolateOpened, {}).tray;
  assert.equal((openingTray.match(/class="chocolate-slot/g) || []).length, 28, 'The ring always renders 28 fixed slots');
  assert.deepEqual([...openingTray.matchAll(/data-slot="(\d+)"/g)].map((match) => Number(match[1])), [...Array.from({ length: 13 }, (_, slot) => slot), 27, 13, ...Array.from({ length: 13 }, (_, offset) => 26 - offset)], 'The picker forms a 13, 2, 13 perimeter loop');
  assert.equal((openingTray.match(/chocolate-ring-gap/g) || []).length, 11, 'The middle row keeps only its two perimeter chocolates');
  assert.equal((openedTray.match(/class="chocolate-slot empty"/g) || []).length, 1, 'A taken chocolate leaves one empty slot instead of shifting the ring');
  assert.match(openedTray, new RegExp(`class="chocolate-slot empty" data-slot="${chosenChocolateIndex}"`), 'The opening chocolate leaves its original slot empty');
  assert.doesNotMatch(openingTray, /queue-label/, 'The chocolate picker has no visible instruction text');
  ...
  assert.match(lobbyHtml, /board\.replaceChildren\(titleZh, image, titleEn\)/, 'Lobby preserves real board screenshots inside labeled decorative backgrounds');
  ...
  assert.equal((lobbyCss.match(/\.game-preview-frame\[data-preview=/g) || []).length, 12, 'Every board has a game-specific background palette');
  assert.match(lobbyHtml, /game-preview-title zh/, 'Lobby backgrounds show the Chinese game name on the left');
  ...
  assert.doesNotMatch(animalLobbySvg, /<text/, 'Animal Shogi board image is label-free; the shared frame overlays the titles like every other card');
  assert.doesNotMatch(animalLobbySvg, /fill="#d7e9c5"/, 'Animal Shogi uses the shared CSS frame background, not a baked-in one');
  ...
  assert.match(shellCss, /\.queue-track \{[^}]*grid-template-columns: repeat\(13, minmax\(0, 1fr\)\);/, 'Chocolate Clash uses a fixed 13, 2, 13 ring layout');
  assert.doesNotMatch(shellCss, /\.queue-track \{[^}]*overflow-x: auto;/, 'Chocolate Clash does not horizontally scroll its ring');
  assert.doesNotMatch(shellCss, /\.chocolate-token\.selected \{[^}]*transform:/, 'Selecting a chocolate does not move it');
  ...
  assert.match(shellCss, /\.memory-mode-icon\.public \{ border: 4px solid currentColor; \}/, 'Public memory mode uses a hollow-circle icon');
  assert.match(shellCss, /\.memory-mode-icon\.hidden, \.memory-mode-icon\.hint \{ background: currentColor; \}/, 'Hidden and hint memory modes use solid-circle icons');
  assert.match(shellCss, /\.memory-mode-icon\.hint::after/, 'Hint memory mode adds a small top-right circle');
  ...
  assert.match(coreSource, /opening: game\.defaultOpening \|\| game\.openings\[0\]\.value/, 'Games may choose a default opening without changing option order');
  assert.match(coreSource, /game\.openingLabel \|\| '棋盤設置'/, 'Games may label the opening setting for their own setup terminology');
