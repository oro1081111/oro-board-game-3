...
(async () => {
  ...
  assert.match(lobbyHtml, /game-preview-title zh/, 'Lobby backgrounds show the Chinese game name at top left');
  assert.match(lobbyHtml, /game-preview-title en/, 'Lobby backgrounds show the English game name at bottom right');
  ...
  assert.match(animalLobbySvg, /fill="#e8ddc2"/, 'Animal Shogi lobby preview follows its cover palette');
  assert.match(animalLobbySvg, />動物將棋<|>LET’S CATCH</, 'Animal Shogi lobby preview labels its background in Chinese and English');
