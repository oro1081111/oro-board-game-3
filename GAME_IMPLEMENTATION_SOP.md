...
- 共用控制器以舊、新 DOM 位置做 FLIP 動畫；一般移動至少 260ms，不能先刪除舊棋再讓新棋無過渡出現。
- 有明確路徑的行動需把 `path` 寫進 action。Torii 與四步棋依 `path` 逐格移動，不只從起點直飛終點。
...
- 花園棋：深、淺雙方剩餘棋子始終並排顯示；只高光並開放當前玩家。棋子外形是方牌，符號區分十字、交叉、內圈、外圈。
- 四色棋：棋子填色代表紅、藍、黃、綠種類；黑方使用黑框、白方使用白框。焦點外框與玩家外框是不同視覺層。
- 四步棋：先選步數再選合法終點；動畫依實際每一步路徑前進，完成後起點格消失。
- 聖托里尼：先選工人、再選移動格、最後選建築格；工人使用穩定 ID 位移，建築層由下往上出現，第三層穹頂沿用 2.0 配色。
- 殭屍棋：紅、藍雙方陰間始終並排顯示；選陰間棋後才標亮復活格，選場上棋後才標亮移動／跳躍格。可跳出時在所選棋子的棋盤邊緣顯示方向按鈕。
- 跳躍森靈：雙方 1、2、3 板塊始終並排顯示，換手時切換高光；先選板塊，再逐步選路徑，森靈沿完整路徑移動，信徒與鳥居各自使用進場動畫。
