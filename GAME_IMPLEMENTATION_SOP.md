...
- 共用控制器以舊、新 DOM 位置做 FLIP 動畫；單格移動以 220ms 至 420ms 為基準，不能先刪除舊棋再讓新棋無過渡出現。
- 有明確路徑的行動需把 `path` 寫進 action。Torii 每次選一步就先播放 220ms；四步棋依 action 的完整 `path` 以每格 280ms 前進，不只從起點直飛終點。
...
- 花園棋：深、淺雙方剩餘棋子始終並排，以區塊與棋子顏色區分，不顯示「深色剩餘／淺色剩餘」文字。尚未選棋前就高光當前可落格；棋子外形是方牌，符號區分十字、交叉、內圈、外圈。
- 蒐靈祭：一般空格不加合法格高光；只有玩家必須從多條得分線中選擇時，才顯示必要的選擇提示。
- 四色棋：棋子填色代表紅、藍、黃、綠種類；黑方使用黑框、白方使用白框。焦點外框與玩家外框是不同視覺層；不顯示頂部總合法步分數區。
- 四步棋：先選步數再選合法終點；動畫依實際每一步路徑以每格 280ms 前進，完成後起點格消失。
- 聖托里尼：先選工人、再選移動格、最後選建築格；選定移動格後，工人必須先以穩定 ID 移到該格，畫面才進入建築選擇。建築層由下往上出現，第三層穹頂沿用 2.0 配色。
- 殭屍棋：紅、藍雙方陰間始終並排，以顏色區分而不顯示陣營文字。陰間棋為正方形，同等級有兩枚以上時只顯示一枚並加數量小標；陰間棋保留穩定 ID，復活時直接動畫到棋盤。選陰間棋後才標亮復活格，選場上棋後才標亮移動／跳躍格。可跳出時在所選棋子的棋盤邊緣顯示方向按鈕。
- 跳躍森靈：雙方 1、2、3 板塊始終並排顯示，換手時切換高光；先選板塊，再逐步選路徑，每選一步森靈就以 220ms 移到該格，最後一步完成後才提交行動。信徒與鳥居各自使用進場動畫。
